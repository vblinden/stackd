...
- `stackd env` writes Next.js `.env` / `.env.local` files, including Prisma-style `DATABASE_URL`, `REDIS_URL`, and SMTP/`EMAIL_*` keys.

### Fixed

- Docker PostgreSQL 18 mounts data at `/var/lib/postgresql` instead of the removed `/var/lib/postgresql/data` path, so `stackd create postgresql` starts successfully.
- Docker start failures now include recent container logs instead of only "exited before it became ready".
...
[1.2.0]: https://github.com/vblinden/stackd/compare/v1.1.1...v1.2.0
